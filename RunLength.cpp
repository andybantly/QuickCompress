// RunLength.cpp : Implementation of CRunLength
#include "stdafx.h"
...
STDMETHODIMP CRunLength::InterfaceSupportsErrorInfo(REFIID riid)
{
...
DWORD CRunLength::EncodedRunLength(BYTE *pInput,DWORD dwCount)
{
	// Compressed Count
	DWORD	dwNewCount = 4;
...
DWORD CRunLength::RunLengthEncode(BYTE *pInput,DWORD dwCount,BYTE *pOutput)
{
	// Compressed Count
	DWORD	dwNewCount = 4;
...
DWORD CRunLength::DecodedRunLength(BYTE *pInput)
{
	// Compressed Count
	DWORD	dwCompCount;
...
DWORD CRunLength::RunLengthDecode(BYTE *pInput,BYTE *pOutput)
{
	// Compressed Count
	DWORD	dwCount;
	DWORD	dwCompCount;
...
STDMETHODIMP CRunLength::Encode(BYTE* pByteBufferIn, UINT nbByteBufferIn, BYTE** ppByteBufferOut, UINT* pnbByteBufferOut)
{
//	DWORD dwCount = SysStringByteLen(ByteBufferIn);
	DWORD dwRunLength = EncodedRunLength(pByteBufferIn, nbByteBufferIn);
//	*pByteBufferOut = SysAllocStringByteLen(NULL,dwRunLength);
//	RunLengthEncode((BYTE*)ByteBufferIn,dwCount,(BYTE*)*pByteBufferOut);
	return S_OK;
}

STDMETHODIMP CRunLength::Decode(BYTE* pByteBufferIn, UINT nbByteBufferIn, BYTE** ppByteBufferOut, UINT* pnbByteBufferOut)
{
//	MessageBoxA(NULL, "Debug", "Debug", MB_OK);
//	DWORD dwCount = DecodedRunLength((BYTE*)ByteBufferIn);
//	*pByteBufferOut = SysAllocStringByteLen(NULL,dwCount);
//	RunLengthDecode((BYTE*)ByteBufferIn,(BYTE*)*pByteBufferOut);
	return S_OK;
}
