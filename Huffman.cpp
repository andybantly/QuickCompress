// Huffman.cpp : Implementation of CHuffman
#include "stdafx.h"
...
// Weight, Byte, Count, Symbol, and Tree Arrays
DWORD dwWeight[256];
DWORD dwCounts[256];
DWORD dwByte[256];
DWORD dwCodes[512];
...
// Build the Byte and Count Arrays, only use Bytes with Weights > 0
void CHuffman::BuildArrays()
{
   __asm
   {
      // Point to the Weight Array
...
_BuildByte:
      ...
      // Get a Count for the Byte
      LODSD
         
      // Compare for a Valid Count
      CMP		EAX,0
      
      // Skip the Byte for a Count of Zero
      JZ		_SkipByte
      
      // The Byte is 256 - Loop 
      MOV		EAX,256
      SUB		EAX,ECX
      ...
      // Loop Back and Get another Byte's Count
_SkipByte:
      DEC		ECX
      JNZ		_BuildByte
...
_BuildCount:
      ...
      // Loop Back and Get another Byte's Weight
_SkipCount:
      DEC		ECX
      JNZ		_BuildCount
...
void CHuffman::BuildByteTree()
{
   ...
   while (iElements > 1)
   {
      ...
      QuickSortD(&dwCounts[0],&dwByte[0],0,iElements - 1);
...
void CHuffman::QuickSortA(DWORD *pArray1,DWORD *pArray2,int iBegin,int iEnd)
{
...
_NoSwap:
   ...
   if (iBegin < iHigh)
      QuickSortA(pArray1,pArray2,iBegin,iHigh);
   
   // Check for Sorting Elements between Low and End (New Upper Range)
   if (iLow < iEnd)
      QuickSortA(pArray1,pArray2,iLow,iEnd);
}

// Quick Sort the Array in Descending Order
void CHuffman::QuickSortD(DWORD *pArray1,DWORD *pArray2,int iBegin,int iEnd)
{
   int		iLow,iHigh,iTemp;
...
_NoSwap:
   ...
   // Check for Sorting Elements between Begin and High (New Lower Range)
   if (iBegin < iHigh)
      QuickSortD(pArray1,pArray2,iBegin,iHigh);
   
   // Check for Sorting Elements between Low and End (New Upper Range)
   if (iLow < iEnd)
      QuickSortD(pArray1,pArray2,iLow,iEnd);
}

// Navigate the Binary Tree
void CHuffman::BuildCodes()
{
   DWORD	dwCodeBits;
   DWORD	dwLeafNode;
...
WORD CHuffman::Dictionary(BYTE *pInput,DWORD dwCount,DWORD *pByteTree,DWORD *pCodes)
{
...
_SumOccurences:
   ...
   // Copy the Byte Tree
   MemoryCopy(pByteTree,&dwByteTree[0],dwStorage);
   
   // Build the Code List
   BuildCodes();
   
   // Copy the Codes
   MemoryCopy(pCodes,&dwCodes[0],2048);
...
DWORD CHuffman::CountCompress(BYTE *pInput,DWORD dwCount,WORD iTreeSize,DWORD *pCodes)
{
...
DWORD CHuffman::Compress(BYTE *pInput,DWORD dwCount,WORD iTreeSize,DWORD *pByteTree,DWORD *pCodes,BYTE *pOutput)
{
...
__Exit:
   ...
   // Copy the Byte Tree
   MemoryCopy(pOutput,&wByteTree[0],dwStorage/2);
...
DWORD CHuffman::GetSize(BYTE *pInput)
{
   DWORD	dwUnCount;
...
DWORD CHuffman::UnCompress(BYTE *pInput,BYTE *pOutput)
{
   // UnCompressed Count
   DWORD	dwUnCount;
   ...
   MemoryCopy(&wByteTree[0],pInput,dwStorage/2);
...
void CHuffman::D2W(void* dest,void* src,DWORD count)
{
   __asm
   {
...
void CHuffman::W2D(void* dest,void* src,DWORD count)
{
   __asm
   {
      MOV		ESI,src		// Copy the Source Address to the Register
...
void CHuffman::MemoryCopy(void* pDest,void* pSrc,DWORD dwCount)
{
   DWORD	iCount;
   ...
   __asm
   {
      ...
      MOV		ESI,pSrc		// Copy the Source Address to the Register
      MOV		EDI,pDest	// Copy the Destination to the Register
...
STDMETHODIMP CHuffman::Compress(BYTE* pByteBufferIn, BYTE** ppByteBufferOut)
{
//    WORD    wts;
//    DWORD dwLength;
//    DWORD	dwbt[768];
//    DWORD	dwc[514];
//    DWORD dwCount = SysStringByteLen(ByteBufferIn);
//    wts = Dictionary((BYTE*)ByteBufferIn,dwCount,&dwbt[0],&dwc[0]);
//    dwLength = CountCompress((BYTE*)ByteBufferIn,dwCount,wts,&dwc[0]);
//    *pByteBufferOut = SysAllocStringByteLen(NULL,dwLength);
//    Compress((BYTE*)ByteBufferIn,dwCount,wts,&dwbt[0],&dwc[0],(BYTE*)*pByteBufferOut);
    return S_OK;
}

STDMETHODIMP CHuffman::Uncompress(BYTE* pByteBufferIn, BYTE** ppByteBufferOut)
{
//    DWORD dwUnCount = GetSize((BYTE*)ByteBufferIn);
//    *pByteBufferOut = SysAllocStringByteLen(NULL,dwUnCount);
//    DWORD dwLength = UnCompress((BYTE*)ByteBufferIn,(BYTE*)*pByteBufferOut);
    return S_OK;
}
